...
int getValidDni(char requestMessage[],char errorMessage[], char errorMessageLenght[],char input[], int maxLenght,int attemps)
{
    ...
    for(i=0;i<attemps;i++)
    {
        if (!getStringDni(requestMessage,buffer))
        {
            printf ("%s",errorMessage);
            continue;
        }
...
/**
 * \brief Valida dni
 * \param arrayDni array a analizar
...
int getValidCuit(char requestMessage[],char errorMessage[], char errorMessageLenght[],char input[], int maxLenght,int attemps)
{
    ...
    for(i=0;i<attemps;i++)
    {
        if (!getStringCuit(requestMessage,buffer))
        {
            printf ("%s",errorMessage);
            continue;
        }
...
{
    char cuit[50];
    getString(mensaje,cuit);
    if(validarCuit(cuit) == 0)
    {
        strcpy(input,cuit);
        return 1;
    }
 ...
 *
 */
int validarCuit(char* arrayCuit)
{
    int i=0;
    int retorno = 0;
    ...
    while(arrayCuit[i] != '\0')
    {
        if(arrayCuit[i] < '0' || arrayCuit[i] > '9' || strlen(arrayCuit)>12 || strlen(arrayCuit)<8)
        {
            retorno = -1;
            break;
        }
